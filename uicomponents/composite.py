...
class Composite(View):
    ...
    def __setattr__(self, key, value):
        try:
            if key in Container.layout_attributes or object.__getattr__(
                    self, key):
    ...
    def shadow_color(self, color):
        ...
        objc_color = ObjCClass('UIColor').colorWithRed_green_blue_alpha_(
            red, green, blue, alpha).CGColor()
...
class Container():
    ...
    layout_attributes = {
        'bounds', 'center', 'flex', 'frame', 'height', 'hidden', 'name',
        'size_to_fit', 'transform', 'width', 'x', 'y'
    }
    ...
    def __init__(self, bottom_view, *views):
        ...
        for view_spec in views:
            ...
            if not isinstance(previous, SelfLayout):
                ...
                object.__setattr__(previous, 'size_to_fit',
                                   as_method(size_right, previous))
                object.__setattr__(previous, 'layout',
                                   as_method(layout_right, previous))
...
class MarginView(SelfLayout):
  ...
  """
    ...
    def layout(self):
        child = self.subviews[0]
        child.frame = (self.left, self.top,
...
class BlurView(View):
    ...
    def setup_effect_view(self):
        ...
        frame = (self.bounds[0], self.bounds[1]), (self.bounds[2],
                                                   self.bounds[3])
        self.effect_view = UIVisualEffectView.alloc().initWithFrame_(
            frame).autorelease()
...
class Notable(Composite):
    ...
    def highlight_negative(self,
                           start_color=(1.0, .63, .63),
                           end_color='white'):
        self.highlight_positive(start_color=start_color, end_color=end_color)

    def pulse_negative(self,
                       start_color=(1.0, .63, .63),
                       end_color='white',
                       duration=1.0):
        self.pulse_positive(
            start_color=start_color, end_color=end_color, duration=duration)

    def highlight_attention(self,
                            start_color=(.57, .72, 1.0),
                            end_color='white'):
        self.highlight_positive(start_color=start_color, end_color=end_color)
...
if __name__ == '__main__':
    ...
    class FancyLabel(Notable, Sized, Rounded, Solid, DefaultLabel):
        pass

    lbl = FancyLabel(
        text='#2 - Size-to-fit label with margins and rounded corners')
    ...
    lbl = SizedSemitransparentRotatedLabel(
        text='#4 - Rotated semi-transparent composite')
    ...
    blur_lbl = BlurRoundLabelButton(
        text='#6 - Blur button', action=click_action)
